...
- JSON 状态概览：`python main.py --status --status-json`（包含 `heartbeat_file` 字段，便于外部探针读取路径）

- 全链路演练（不发通知、不写历史）：`python main.py --once --dry-run`

- 单轮 JSON 结果：`python main.py --once --once-json`（包含 `processed`、`dry_run`、`status`、`timestamp`；异常时含 `error` 并返回非零退出码）

- 心跳文件示例：`HEARTBEAT_FILE=heartbeat.json`（每轮更新状态）。

- 有限轮询运行：`python main.py --max-cycles 3 --dry-run`

- 参数说明：`--max-cycles` 必须是 >=0 的整数；`--once-json` 仅在 `--once` 时生效；`--status-json` 仅在 `--status` 时生效；`--preflight-json` 仅在 `--preflight` 时生效。
